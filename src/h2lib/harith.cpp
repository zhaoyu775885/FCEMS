...
static void
trunc_ab_rkmatrix(pctruncmode tm, double eps, prkmatrix r)
{
  ...
  copy_sub_amatrix(false, u, &r->U);
  copy_sub_amatrix(true, vt, &r->V);

  /* Scale singular vectors */
  diageval_realavector_amatrix(1.0, true, sigma, true, &r->U);
 ...
 * rank. */
static void
trunc_qb_rkmatrix(pctruncmode tm, double eps, prkmatrix r)
{
  ...
  /* Copy singular vectors */
  copy_sub_amatrix(false, u, &r->V);
  clear_amatrix(&r->U);
  copy_sub_amatrix(true, vt, &r->U);
  qreval_amatrix(false, a, tau, &r->U);
  uninit_avector(tau);

  /* Scale singular vectors */
  diageval_realavector_amatrix(1.0, true, sigma, true, &r->V);
 ...
 * rank. */
static void
trunc_aq_rkmatrix(pctruncmode tm, double eps, prkmatrix r)
{
  amatrix   tmp1, tmp2, tmp3, tmp4;
  avector   tmp5;
  realavector tmp6;
  ...
  clear_amatrix(&r->V);
  copy_sub_amatrix(true, vt, &r->V);
  qreval_amatrix(false, b, tau, &r->V);
  uninit_avector(tau);
...
static void
trunc_qq_rkmatrix(pctruncmode tm, double eps, prkmatrix r)
{
  ...
  qreval_amatrix(false, a, atau, &r->U);
  qreval_amatrix(false, b, btau, &r->V);
...
void
trunc_rkmatrix(pctruncmode tm, double eps, prkmatrix r)
{
  int      rows, cols, k;
...
add_amatrix_rkmatrix(field alpha, bool atrans, pcamatrix a, pctruncmode tm,
		     double eps, prkmatrix b)
{
  amatrix   tmp1, tmp2, tmp3, tmp4;
  realavector tmp5;
  pamatrix  z, u, vt, u1, vt1;
  prealavector sigma;
  int      rows, cols, k, knew;
  ...
  if (atrans) {
    assert(a->rows == b->V.rows);
    assert(a->cols == b->U.rows);
  }
  else {
    assert(a->rows == b->U.rows);
    assert(a->cols == b->V.rows);
  }
  ...
  addmul_amatrix(1.0, false, &b->U, true, &b->V, z);
  ...
  copy_amatrix(false, u1, &b->U);
  uninit_amatrix(u1);

  clear_amatrix(&b->V);
  vt1 = init_sub_amatrix(&tmp4, vt, knew, 0, cols, 0);
  copy_amatrix(true, vt1, &b->V);
  uninit_amatrix(vt1);
...
add_rkmatrix_amatrix(field alpha, bool atrans, pcrkmatrix a, pamatrix b)
{
  ...
  if (atrans) {
    assert(a->V.rows == b->rows);
    assert(a->U.rows == b->cols);

    if (a->k > 0)
      addmul_amatrix(alpha, false, &a->V, true, &a->U, b);
  }
  else {
    assert(a->U.rows == b->rows);
    assert(a->V.rows == b->cols);

    if (a->k > 0)
      addmul_amatrix(alpha, false, &a->U, true, &a->V, b);
  }
}
		...
		double eps, prkmatrix trg)
{
  ...
  assert(src->U.rows == rows);
  assert(src->V.rows == cols);
  ...
  /* Determine rank */
  knew = findrank_truncmode(tm, eps, sigma);
  //cout << k1 << " rank = " << knew << endl;

  /* Set new rank */
  setrank_rkmatrix(trg, knew);

  /* Copy singular vectors */
  copy_sub_amatrix(false, u, &trg->U);
  copy_sub_amatrix(true, vt, &trg->V);

  /* Scale singular vectors */
  diageval_realavector_amatrix(1.0, true, sigma, true, &trg->U);
...
void
add_ab_rkmatrix_keep_rank(field alpha, pcrkmatrix src, prkmatrix trg)
{
  amatrix   tmp1, tmp2, tmp3, tmp4, tmp5;
  realavector tmp6;
  pamatrix  a, b, c, a1, b1, u, vt;
		...
		double eps, prkmatrix trg)
{
  ...
  /* Copy singular vectors */
  copy_sub_amatrix(false, u, &trg->V);
  clear_amatrix(&trg->U);
  copy_sub_amatrix(true, vt, &trg->U);
  qreval_amatrix(false, a, tau, &trg->U);
  uninit_avector(tau);

  /* Scale singular vectors */
  diageval_realavector_amatrix(1.0, true, sigma, true, &trg->V);
...
void
add_qb_rkmatrix_keep_rank(field alpha, pcrkmatrix src, prkmatrix trg)
{
  amatrix   tmp1, tmp2, tmp3, tmp4, tmp5;
  avector   tmp6;
  realavector tmp7;
		...
		double eps, prkmatrix trg)
{
  ...
  clear_amatrix(&trg->V);
  copy_sub_amatrix(true, vt, &trg->V);
  qreval_amatrix(false, b, tau, &trg->V);
  uninit_avector(tau);
...
void
add_aq_rkmatrix_keep_rank(field alpha, pcrkmatrix src, prkmatrix trg)
{
  amatrix   tmp1, tmp2, tmp3, tmp4, tmp5;
  avector   tmp6;
  realavector tmp7;
		...
		double eps, prkmatrix trg)
{
  ...
  /* Multiply singular vectors by Q_A and Q_B, respectively */
  qreval_amatrix(false, a, atau, &trg->U);
  qreval_amatrix(false, b, btau, &trg->V);
...
void
add_qq_rkmatrix_keep_rank(field alpha, pcrkmatrix src, prkmatrix trg)
{
  amatrix   tmp1, tmp2, tmp3, tmp4, tmp5;
  avector   tmp6, tmp7;
  realavector tmp8;
...
//  ptruncmode tm = new_truncmode();
  knew = trg->k;//findrank_truncmode(tm, 1e-2, sigma);
//  cout << "the found rank is : " << knew << endl;
...
void
add_rkmatrix_keep_rank(field alpha, pcrkmatrix src, prkmatrix trg)
{
  int      rows, cols, k;
  ...
  if (k < rows) {
    if (k < cols) {
      /* rows and cols large, use QR decomposition for A and B */
      add_qq_rkmatrix_keep_rank(alpha, src, trg);
    }
    else {
      add_qb_rkmatrix_keep_rank(alpha, src, trg);
    }
  }
  else {
    if (k < cols) {
      /* rows small, cols large, use QR decomposition for B only */
      add_aq_rkmatrix_keep_rank(alpha, src, trg);
    }
    else {
      /* rows and cols small, no QR decomposition required */
      add_ab_rkmatrix_keep_rank(alpha, src, trg);
    }
  }
}
 ...
 * considering the rank, number of rows and number of columns. */
void
add_rkmatrix(field alpha, pcrkmatrix src, pctruncmode tm, double eps,
	     prkmatrix trg)
{
  int      rows, cols, k;
...
add_lower_amatrix_hmatrix(field alpha, bool atrans, pcamatrix a,
			  pctruncmode tm, double eps, phmatrix b)
{
  amatrix   tmp;
  pamatrix  a1;
  phmatrix  b1;
...
void
add_rkmatrix_hmatrix(field alpha, pcrkmatrix r, pctruncmode tm, double eps,
		     phmatrix a)
{
  rkmatrix  tmp;
  phmatrix  a1;
  ...
  if (a->r)
    ...
    addmul_amatrix(alpha, false, &r->U, true, &r->V, a->f);
  else {
    rsons = a->rsons;
    csons = a->csons;
...
void
add_hmatrix(field alpha, pchmatrix a, pctruncmode tm, double eps, phmatrix b)
{
  phmatrix  bs;
  prkmatrix rk;
  int      rsons, csons;
		  ...
		  bool csplit)
{
  ...
  if (rsplit && rc->nsons > 0) {
    rsons = rc->nsons;

    if (csplit && cc->nsons > 0) {
		  ...
		  double eps, prkmatrix trg)
{
  ...
  pavector  tau1, tau2;
  prealavector sigma;
  int      rows, cols, rows1, rows2, cols1, cols2;
  int      k, k1, k2, kk, kmax, knew;

  if (colmerge) {
    assert(src->V.rows == trg->V.rows);
    ...
    copy_amatrix(false, &src->V, b2);
    uninit_amatrix(b2);

    /* Compute factorization A1 = Q1 R1 */
    k1 = UINT_MIN(trg->U.rows, trg->k);
    a1 = init_amatrix(&tmp3, trg->U.rows, trg->k);
    copy_amatrix(false, &trg->U, a1);
    tau1 = init_avector(&tmp7, k1);
    qrdecomp_amatrix(a1, tau1);

    /* Compute factorization A2 = Q2 R2 */
    k2 = UINT_MIN(src->U.rows, src->k);
    a2 = init_amatrix(&tmp4, src->U.rows, src->k);
    copy_amatrix(false, &src->U, a2);
    ...
    v1 = init_sub_amatrix(&tmp1, &trg->U, k1, 0, knew, 0);
    copy_amatrix(true, vt1, v1);
    uninit_amatrix(v1);
    uninit_amatrix(vt1);

    vt1 = init_sub_amatrix(&tmp6, vt, knew, 0, k2, k1);
    v1 = init_sub_amatrix(&tmp1, &trg->U, k2, rows1, knew, 0);
    copy_amatrix(true, vt1, v1);
    uninit_amatrix(v1);
    uninit_amatrix(vt1);
    ...
    v1 = init_sub_amatrix(&tmp1, &trg->U, rows1, 0, knew, 0);
    qreval_amatrix(false, a1, tau1, v1);
    uninit_amatrix(v1);

    v1 = init_sub_amatrix(&tmp1, &trg->U, rows2, rows1, knew, 0);
    ...
    k1 = UINT_MIN(trg->V.rows, trg->k);
    b1 = init_amatrix(&tmp3, trg->V.rows, trg->k);
    copy_amatrix(false, &trg->V, b1);
    tau1 = init_avector(&tmp7, k1);
    qrdecomp_amatrix(b1, tau1);

    /* Compute factorization B2 = Q2 R2 */
    k2 = UINT_MIN(src->V.rows, src->k);
    b2 = init_amatrix(&tmp4, src->V.rows, src->k);
    copy_amatrix(false, &src->V, b2);
    tau2 = init_avector(&tmp8, k2);
    ...
    vt1 = init_sub_amatrix(&tmp6, vt, knew, 0, k1, 0);
    v1 = init_sub_amatrix(&tmp1, &trg->V, k1, 0, knew, 0);
    copy_amatrix(true, vt1, v1);
    uninit_amatrix(v1);
    uninit_amatrix(vt1);

    vt1 = init_sub_amatrix(&tmp6, vt, knew, 0, k2, k1);
    v1 = init_sub_amatrix(&tmp1, &trg->V, k2, cols1, knew, 0);
    copy_amatrix(true, vt1, v1);
    uninit_amatrix(v1);
    uninit_amatrix(vt1);
    ...
    /* Apply orthogonal transformations */
    v1 = init_sub_amatrix(&tmp1, &trg->V, cols1, 0, knew, 0);
    qreval_amatrix(false, b1, tau1, v1);
    uninit_amatrix(v1);

    v1 = init_sub_amatrix(&tmp1, &trg->V, cols2, cols1, knew, 0);
...
void
merge_rkmatrix(bool colmerge, pcrkmatrix src, pctruncmode tm, double eps,
	       prkmatrix trg)
{
...
prkmatrix
merge_hmatrix_rkmatrix(pchmatrix s, pctruncmode tm, double eps)
{
  rkmatrix  tmp1;
  prkmatrix r1, r;
  pcrkmatrix s1;
  ...
  if (rsons > 1) {
	...
	r1 = init_rkmatrix(&tmp1, s1->U.rows, s1->V.rows, s1->k);
	...
	uninit_rkmatrix(r1);
      ...
      r = new_rkmatrix(s1->U.rows, s1->V.rows, s1->k);
				...
				pamatrix z)
{
  ...
  if (xtrans) {
    ...
    clear_amatrix(ay);
    addmul_amatrix(1.0, true, &x->U, ytrans, y, ay);

    if (ztrans)
      addmul_amatrix(CONJ(alpha), true, ay, true, &x->V, z);
    else
      addmul_amatrix(alpha, false, &x->V, false, ay, z);
    ...
    clear_amatrix(by);
    addmul_amatrix(1.0, true, &x->V, ytrans, y, by);

    if (ztrans)
      addmul_amatrix(CONJ(alpha), true, by, true, &x->U, z);
    else
      addmul_amatrix(alpha, false, &x->U, false, by, z);
 ...
 * static function. */
void
addmul_hmatrix_amatrix_amatrix(field alpha, bool atrans, pchmatrix a, bool btrans, pcamatrix bp, bool ctrans, pamatrix cp)
{
...
addmul_nn_hmatrix(field alpha, pchmatrix x, pchmatrix y,
		  pctruncmode tm, double eps, phmatrix z)
{
  prkmatrix xy;
  pamatrix  id;
  pcamatrix xf, yf;
  ...
  if (x->f) {
	...
	copy_amatrix(false, xf, &xy->U);
	id = init_amatrix(&tmp2, xf->cols, xf->cols);
	identity_amatrix(id);
	clear_amatrix(&xy->V);
	addmul_hmatrix_amatrix_amatrix(CONJ(alpha), true, y, true, id, false, &xy->V);
	...
	identity_amatrix(&xy->U);
	clear_amatrix(&xy->V);
	addmul_hmatrix_amatrix_amatrix(CONJ(alpha), true, y, true, xf, false, &xy->V);
    ...
    copy_amatrix(false, &x->r->U, &xy->U);
    clear_amatrix(&xy->V);
    addmul_hmatrix_amatrix_amatrix(CONJ(alpha), true, y, false, &x->r->V,
				   false, &xy->V);
    ...
    if (y->f) {
      yf = y->f;
      if (z->f)			/* Z = Z + X Y */
		  addmul_hmatrix_amatrix_amatrix(alpha, false, x, false, yf, false, z->f);
	  else {
		  if (yf->cols > yf->rows) {
			  /* Compute rkmatrix X Y = (X I) (Y^*)^* */
			   xy = init_rkmatrix(&tmp1, rc->size, cc->size, yf->rows);
			   copy_amatrix(true, yf, &xy->V);
			   id = init_amatrix(&tmp2, yf->rows, yf->rows);
			   identity_amatrix(id);
			   clear_amatrix(&xy->U);
			   addmul_hmatrix_amatrix_amatrix(alpha, false, x, false, id, false, &xy->U);
			   /* Add rkmatrix to Z */
			   add_rkmatrix_hmatrix(1.0, xy, tm, eps, z);
			   uninit_amatrix(id);
			   uninit_rkmatrix(xy);
			  ...
			  identity_amatrix(&xy->V);

			  clear_amatrix(&xy->U);

			  addmul_hmatrix_amatrix_amatrix(alpha, false, x, false, yf, false, &xy->U);
      ...
      copy_amatrix(false, &y->r->V, &xy->V);
      clear_amatrix(&xy->U);
      addmul_hmatrix_amatrix_amatrix(alpha, false, x, false, &y->r->U, false,
				     &xy->U);
	...
	if (y->son[0]->cc == y->cc) {
	  ...
	  if (x->son[0]->rc == x->rc) {
	    ...
	    for (k = 0; k < csons; k++) {
	      ...
	      for (i = 0; i < rsons; i++) {
		/* Create submatrix of z */
		zf1 = init_sub_amatrix(&tmp2, zf, rc->son[i]->size, roff, cc->son[k]->size, coff);
		z1 = init_hmatrix(&tmp3, rc->son[i], cc->son[k]);
		z1->f = zf1;

		/* Multiplications of submatrices */
		for (j = 0; j < msons; j++)
		  addmul_nn_hmatrix(alpha, x->son[i + j * rsons], y->son[j + k * msons], tm, eps, z1);
	...
	for (k = 0; k < csons; k++)
	  for (i = 0; i < rsons; i++)
	    for (j = 0; j < msons; j++)
	      addmul_nn_hmatrix(alpha, x->son[i + j * rsons], y->son[j + k * msons], tm, eps, ztmp->son[i + k * rsons]);
	...
	for (k = 0; k < csons; k++)
	  for (i = 0; i < rsons; i++)
	    for (j = 0; j < msons; j++)
	      addmul_nn_hmatrix(alpha, x->son[i + j * rsons], y->son[j + k * msons], tm, eps, z->son[i + k * rsons]);
      }
    }
  }
}
		  ...
		  pctruncmode tm, double eps, phmatrix z)
{
  ...
  if (x->f) {
    ...
    copy_amatrix(false, &x->r->U, &xy->U);
    clear_amatrix(&xy->V);
    addmul_hmatrix_amatrix_amatrix(CONJ(alpha), false, y, false, &x->r->V,
				   false, &xy->V);
	...
	if (yf->cols < yf->rows) {
      ...
      xy = init_rkmatrix(&tmp1, rc->size, cc->size, y->r->k);
      copy_amatrix(false, &y->r->U, &xy->V);
      clear_amatrix(&xy->U);
      addmul_hmatrix_amatrix_amatrix(alpha, false, x, false, &y->r->V, false,
				     &xy->U);
		  ...
		  pctruncmode tm, double eps, phmatrix z)
{
  ...
  if (x->f) {
    ...
    xy = init_rkmatrix(&tmp1, rc->size, cc->size, x->r->k);
    copy_amatrix(false, &x->r->V, &xy->U);
    clear_amatrix(&xy->V);
    addmul_hmatrix_amatrix_amatrix(CONJ(alpha), true, y, false, &x->r->U,
				   false, &xy->V);
	...
	if (yf->cols > yf->rows) {
      ...
      copy_amatrix(false, &y->r->V, &xy->V);
      clear_amatrix(&xy->U);
      addmul_hmatrix_amatrix_amatrix(alpha, true, x, false, &y->r->U, false,
				     &xy->U);
		  ...
		  pctruncmode tm, double eps, phmatrix z)
{
  ...
  if (x->f) {
    ...
    copy_amatrix(false, &x->r->V, &xy->U);
    clear_amatrix(&xy->V);
    addmul_hmatrix_amatrix_amatrix(CONJ(alpha), false, y, false, &x->r->U,
				   false, &xy->V);
	...
	if (yf->cols < yf->rows) {
      ...
      copy_amatrix(false, &y->r->U, &xy->V);
      clear_amatrix(&xy->U);
      addmul_hmatrix_amatrix_amatrix(alpha, true, x, false, &y->r->V, false,
				     &xy->U);
...
addmul_hmatrix(field alpha, bool xtrans, pchmatrix x, bool ytrans,
	       pchmatrix y, pctruncmode tm, double eps, phmatrix z)
{
  if (xtrans) {
    if (ytrans)
		     ...
		     pchmatrix y, pctruncmode tm, double eps, phmatrix z)
{
  ...
  if (x->f) {
	...
	copy_amatrix(true, xf, &mr->U);
	clear_amatrix(&mr->V);
	add_hmatrix_amatrix(1.0, !ytrans, y, &mr->V);
	...
	mr = init_rkmatrix(&tmp2, (xtrans ? x->cc->size : x->rc->size),
			   yf->rows, yf->cols);
	clear_amatrix(&mr->U);
	add_hmatrix_amatrix(1.0, xtrans, x, &mr->U);
	copy_amatrix(false, yf, &mr->V);
	add_lower_rkmatrix_hmatrix(alpha, mr, tm, eps, z);
	uninit_rkmatrix(mr);
      }
    }
    ...
    if (xtrans) {
      ...
      copy_amatrix(false, &xr->V, &mr->U);
      clear_amatrix(&mr->V);
      addmul_hmatrix_amatrix_amatrix(1.0, !ytrans, y, false, &xr->U, false,
				     &mr->V);
      add_lower_rkmatrix_hmatrix(alpha, mr, tm, eps, z);
      uninit_rkmatrix(mr);
    }
      ...
      mr = init_rkmatrix(&tmp2, x->rc->size,
			 (ytrans ? y->rc->size : y->cc->size), xr->k);
      copy_amatrix(false, &xr->U, &mr->U);
      clear_amatrix(&mr->V);
      addmul_hmatrix_amatrix_amatrix(1.0, !ytrans, y, false, &xr->V, false,
				     &mr->V);
      add_lower_rkmatrix_hmatrix(alpha, mr, tm, eps, z);
      uninit_rkmatrix(mr);
    }
    ...
    if (ytrans) {
      ...
      mr = init_rkmatrix(&tmp2, (xtrans ? x->cc->size : x->rc->size),
			 y->rc->size, yr->k);
      clear_amatrix(&mr->U);
      addmul_hmatrix_amatrix_amatrix(1.0, xtrans, x, false, &yr->V, false,
				     &mr->U);
      copy_amatrix(false, &yr->U, &mr->V);
      add_lower_rkmatrix_hmatrix(alpha, mr, tm, eps, z);
      uninit_rkmatrix(mr);
    }
    else {
      ...
      mr = init_rkmatrix(&tmp2, (xtrans ? x->cc->size : x->rc->size),
			 y->cc->size, yr->k);
      clear_amatrix(&mr->U);
      addmul_hmatrix_amatrix_amatrix(1.0, xtrans, x, false, &yr->U, false,
				     &mr->U);
      copy_amatrix(false, &yr->V, &mr->V);
      add_lower_rkmatrix_hmatrix(alpha, mr, tm, eps, z);
      uninit_rkmatrix(mr);
    }
  }
...
void
invert_hmatrix(phmatrix a, phmatrix work, pctruncmode tm, double eps)
{
  int      sons;
  int      i, j, k;
  ...
  if (a->f)
	...
	addmul_hmatrix(1.0, false, a->son[i + k * sons], false, a->son[k + k * sons], tm, eps, work->son[i + k * sons]);
      }

      /* Compute W_{kj} = A_{kk}^{-1} A_{kj} */
      for (j = k + 1; j < sons; j++) {
	clear_hmatrix(work->son[k + j * sons]);
	addmul_hmatrix(1.0, false, a->son[k + k * sons], false, a->son[k + j * sons], tm, eps, work->son[k + j * sons]);
      }

      /* Update Schur complement A_{ij} = A_{ij} - W_{ik} A_{kj} */
    for (j = k + 1; j < sons; j++)
      for (i = k + 1; i < sons; i++)
       addmul_hmatrix(-1.0, false, work->son[i + k * sons], false, a->son[k + j * sons], tm, eps, a->son[i + j * sons]);
    }
	...
	for (j = k + 1; j < sons; j++)
	  addmul_hmatrix(-1.0, false, a->son[i + j * sons], false, work->son[j + k * sons], tm, eps, a->son[i + k * sons]);
      }
	...
	for (i = k + 1; i < sons; i++)
	  addmul_hmatrix(-1.0, false, work->son[k + i * sons], false, a->son[i + j * sons], tm, eps, a->son[k + j * sons]);
      }

      /* Compute (A^{-1})_{kk} = A_{kk}^{-1} + \sum_{i>k} (A^{-1})_{ki} W_{ik} */
      for (i = k + 1; i < sons; i++)
	addmul_hmatrix(-1.0, false, a->son[k + i * sons], false, work->son[i + k * sons], tm, eps, a->son[k + k * sons]);
    }
  }
}
...
lowersolve_n_hmatrix_avector(bool aunit, pchmatrix a, pavector xp)
{
  ...
  if (a->f)
	...
	fastaddeval_hmatrix_avector(-1.0, a->son[j + i * sons], h2_ntrans, xp1, f_one, xp2);
...
lowersolve_t_hmatrix_avector(bool aunit, pchmatrix a, pavector xp)
{
  ...
  if (a->f)
	...
	fastaddeval_hmatrix_avector(-1.0, a->son[i + j * sons], h2_trans, xp1, f_one, xp2);
...
lowersolve_nn_hmatrix_amatrix(bool aunit, pchmatrix a, pamatrix xp)
{
  ...
  if (a->f)
    ...
    roff = 0;
    for (i = 0; i < sons; i++) {
		xp1 = init_sub_amatrix(&tmp1, xp, a->son[i]->rc->size, roff, xp->cols, 0);
		lowersolve_nn_hmatrix_amatrix(aunit, a->son[i + i * sons], xp1);

		roff2 = roff + a->son[i]->rc->size;

		for (j = i + 1; j < sons; j++) {
			xp2 = init_sub_amatrix(&tmp2, xp, a->son[j]->rc->size, roff2, xp->cols, 0);
			addmul_hmatrix_amatrix_amatrix(-1.0, false, a->son[j + i * sons], false, xp1, false, xp2);
			uninit_amatrix(xp2);
		      ...
		      double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    lowersolve_nn_hmatrix_amatrix(aunit, a, &xp->r->U);
  else {
    if (a->f) {
	...
	for (i = 0; i < sons; i++) {
      ...
      for (k = 0; k < xp->csons; k++)
		  for (i = 0; i < sons; i++) {
			  lowersolve_nn_hmatrix(aunit, a->son[i + i * sons], tm, eps, xp->son[i + k * xp->rsons]);

			  for (j = i + 1; j < sons; j++)
				   addmul_hmatrix(-1.0, false, a->son[j + i * sons], false, xp->son[i + k * xp->rsons], tm, eps, xp->son[j + k * xp->rsons]);
		  }
	}
  }
}
		      ...
		      double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    lowersolve_nn_hmatrix_amatrix(aunit, a, &xp->r->V);
  else {
    if (a->f) {
		      ...
		      double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    lowersolve_tn_hmatrix_amatrix(aunit, a, &xp->r->U);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->rc != xp->rc),
			       (xp->son[0]->rc != xp->rc));
		      ...
		      double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    lowersolve_tn_hmatrix_amatrix(aunit, a, &xp->r->V);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->cc != xp->cc),
			       (xp->son[0]->cc != xp->cc));
...
lowersolve_hmatrix(bool aunit, bool atrans, pchmatrix a,
		   pctruncmode tm, double eps, bool xtrans, phmatrix xp)
{
  if (atrans) {
    if (xtrans)
		      ...
		      double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    uppersolve_nn_hmatrix_amatrix(aunit, a, &xp->r->U);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->rc != xp->rc),
			       (xp->son[0]->rc != xp->rc));
		      ...
		      double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    uppersolve_nn_hmatrix_amatrix(aunit, a, &xp->r->V);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->cc != xp->cc),
			       (xp->son[0]->cc != xp->cc));
		      ...
		      double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    uppersolve_tn_hmatrix_amatrix(aunit, a, &xp->r->U);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->rc != xp->rc),
			       (xp->son[0]->rc != xp->rc));
		      ...
		      double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    uppersolve_tn_hmatrix_amatrix(aunit, a, &xp->r->V);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->cc != xp->cc),
			       (xp->son[0]->cc != xp->cc));
...
lowersolve_nn_amatrix_hmatrix(bool aunit, pcamatrix a,
			      pctruncmode tm, double eps, phmatrix xp)
{
  hmatrix   tmp;
...
triangularinvmul_amatrix_hmatrix(bool alower, bool aunit, bool atrans,
				 pcamatrix a, pctruncmode tm, double eps,
...
lowereval_n_hmatrix_avector(bool aunit, pchmatrix a, pavector xp)
{
  ...
  if (a->f)
	...
	fastaddeval_hmatrix_avector(1.0, a->son[j + i * sons], h2_trans, xp1, f_one, xp2);
		     ...
		     double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    lowereval_n_hmatrix_amatrix(aunit, a, false, &xp->r->U);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->rc != xp->rc),
			       (xp->son[0]->rc != xp->rc));
		     ...
		     double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    lowereval_n_hmatrix_amatrix(aunit, a, false, &xp->r->V);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->cc != xp->cc),
			       (xp->son[0]->cc != xp->cc));
		     ...
		     double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    lowereval_t_hmatrix_amatrix(aunit, a, false, &xp->r->U);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->rc != xp->rc),
			       (xp->son[0]->rc != xp->rc));
		     ...
		     double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    lowereval_t_hmatrix_amatrix(aunit, a, false, &xp->r->V);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->cc != xp->cc),
			       (xp->son[0]->cc != xp->cc));
		     ...
		     double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    uppereval_n_hmatrix_amatrix(aunit, a, false, &xp->r->U);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->rc != xp->rc),
			       (xp->son[0]->rc != xp->rc));
		     ...
		     double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    uppereval_n_hmatrix_amatrix(aunit, a, false, &xp->r->V);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->cc != xp->cc),
			       (xp->son[0]->cc != xp->cc));
		     ...
		     double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    uppereval_t_hmatrix_amatrix(aunit, a, false, &xp->r->U);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->rc != xp->rc),
			       (xp->son[0]->rc != xp->rc));
		     ...
		     double eps, phmatrix xp)
{
  ...
  if (xp->f)
    ...
    uppereval_t_hmatrix_amatrix(aunit, a, false, &xp->r->V);
  else {
    if (a->f) {
      atmp = split_sub_amatrix(a->f, a->rc, a->cc, (xp->son[0]->cc != xp->cc),
			       (xp->son[0]->cc != xp->cc));
...
void
lrdecomp_hmatrix(phmatrix a, pctruncmode tm, double eps)
{
  int      sons;
  int      i, j, k;
  int      res;
  ...
  if (a->f) {
    ...
    for (k = 0; k < sons; k++) {
		...
		hadepth--;
  		...
  		for (j = k + 1; j < sons; j++) {
			lowersolve_hmatrix(true, false, a->son[k + k * sons], tm, eps, false, a->son[k + j * sons]);
		}

  		for (i = k + 1; i < sons; i++) {
			uppersolve_hmatrix(false, true, a->son[k + k * sons], tm, eps, true, a->son[i + k * sons]);
		}
//		cout << "begin addmul" << endl;

  		for (j = k + 1; j < sons; j++)
			for (i = k + 1; i < sons; i++)
				 addmul_hmatrix(-1.0, false, a->son[i + k * sons], false, a->son[k + j * sons], tm, eps, a->son[i + j * sons]);
    }
  }
}
...
void
choldecomp_hmatrix(phmatrix a, pctruncmode tm, double eps)
{
  int      sons;
  int      i, j, k;
  int      res;
